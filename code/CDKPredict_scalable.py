...
class CDKPredict_scalable:
    ...
    def fit(self, X, y):
        ...
        self.feature_prevalence_ = X.mean(axis=0)
    ...
    def predict(self, X, classify = False, groups = None, verbose = True):
        ...
        if X.isnull().values.any():
            if verbose:
                ...
                print(f"Warning: Some samples have missing values. Imputing with training prevalence for features: {missing_features}")
            X = X.fillna(self.feature_prevalence_)
        risk_scores = []
